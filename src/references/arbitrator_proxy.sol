...
interface IRealitio {
    ...
    ) external view returns (uint32, bool, bytes32);
...
abstract contract IDisputeResolver is IArbitrable, IEvidence {
     ...
     */
    event Contribution(
        uint256 indexed _localDisputeID,
    ...
    ) external virtual returns (uint256 localDisputeID);
 ...
 */
abstract contract RealitioArbitratorWithAppealsBase is
    IDisputeResolver,
    IRealitioArbitrator
{
    ...
    function requestArbitration(
        ...
        require(
            arbitrationRequest.status == Status.None,
    ...
    function rule(uint256 _disputeID, uint256 _ruling) public override {
        require(
            IArbitrator(msg.sender) == arbitrator,
        ...
        Round storage round = arbitrationRequest.rounds[
            arbitrationRequest.rounds.length - 1
        ];
        ...
        uint256 finalRuling = (round.fundedRulings.length == 1)
            ? round.fundedRulings[0]
            : _ruling;
    ...
    function fundAppeal(
        ...
        uint256 totalCost;
        {
            (uint256 originalStart, uint256 originalEnd) = arbitrator
            ...
            if (_ruling == currentRuling) {
                ...
                require(
                    block.timestamp <
                        (originalStart + ((originalEnd - originalStart) / 2)),
            ...
            originalCost = arbitrator.appealCost(
                disputeID,
                arbitratorExtraData
            );
            totalCost = originalCost.addCap(
                originalCost.mulCap(multiplier) / MULTIPLIER_DENOMINATOR
            );
        }
        ...
        uint256 contribution = totalCost.subCap(lastRound.paidFees[_ruling]) >
            msg.value
            ? msg.value
        ...
        if (lastRound.fundedRulings.length == 2) {
            ...
            arbitrator.appeal{value: originalCost}(
                disputeID,
                arbitratorExtraData
            );
        }
            ...
            if (_ruling == _finalRuling) {
                ...
                amount = _round.paidFees[_ruling] > 0
                    ? (_round.contributions[_contributor][_ruling] *
                        _round.feeRewards) / _round.paidFees[_ruling]
                    : 0;
